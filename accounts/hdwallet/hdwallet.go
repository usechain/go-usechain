...
func New(password string) (*Wallet, string, error) {
	...
	w, err := NewFromMnemonicAndPassword(password, mnemonic)
...
func (w *Wallet) SelfDerive(base accounts.DerivationPath, chain usechain.ChainStateReader) {
...
// SignTx implements accounts.Wallet, which allows the account to sign an usechain transaction.
...
func (w *Wallet) AccountForIndex(index int) (accounts.Account, error) {
	...
	path := MustParseDerivationPath(p)
