...
// wallets. The wire protocol spec can be found in the Ledger Blue gitlab repo:
// https://raw.gitlabusercontent.com/LedgerHQ/blue-app-eth/master/doc/ethapp.asc
...
func (w *ledgerDriver) Status() (string, error) {
	...
	if w.browser {
		return "Usechain app in browser mode", w.failure
	}
	if w.offline() {
		return "Usechain app offline", w.failure
	}
	return fmt.Sprintf("Usechain app v%d.%d.%d online", w.version[0], w.version[1], w.version[2]), w.failure
}
