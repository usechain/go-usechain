...
import (
	...
	"github.com/usechain/go-usechain/p2p/discover"
	"github.com/usechain/go-usechain/p2p/discv5"
...
var (
	...
	netFlag     = flag.Uint64("network", 0, "Network ID to use for the Usechain protocol")
	...
	gitlabUser  = flag.String("gitlab.user", "", "gitlab user to authenticate with for Gist access")
	gitlabToken = flag.String("gitlab.token", "", "gitlab personal token to access Gists with")

	captchaToken  = flag.String("captcha.token", "", "Recaptcha site key to authenticate client side")
	captchaSecret = flag.String("captcha.secret", "", "Recaptcha secret key to authenticate server side")

	noauthFlag = flag.Bool("noauth", false, "Enables funding requests without authentication")
	logFlag    = flag.Int("loglevel", 3, "Log level to use for Usechain and the faucet")
)
...
func (f *faucet) apiHandler(conn *websocket.Conn) {
	...
	for {
		// Fetch the next funding request and validate against gitlab
		var msg struct {
			URL     string `json:"url"`
			Tier    uint   `json:"tier"`
		...
		case strings.HasPrefix(msg.URL, "https://gist.github.com/"):
			if err = sendError(conn, errors.New("gitlab authentication discontinued at the official request of gitlab")); err != nil {
				log.Warn("Failed to send gitlab deprecation to client", "err", err)
				return
			}
			continue
		...
		default:
			err = errors.New("Something funky happened, please open an issue at https://github.com/usechain/go-usechain/issues")
		}
		if err != nil {
			if err = sendError(conn, err); err != nil {
		...
		if timeout = f.timeouts[username]; time.Now().After(timeout) {
			...
			tx := types.NewTransaction(0, f.nonce+uint64(len(f.reqs)), address, amount, 21000, f.price, nil)
...
// authgitlab tries to authenticate a faucet request using gitlab gists, returning
// the username, avatar URL and Ethereum address to fund on success.
func authgitlab(url string) (string, string, common.Address, error) {
	// Retrieve the gist from the gitlab Gist APIs
	parts := strings.Split(url, "/")
	req, _ := http.NewRequest("GET", "https://api.github.com/gists/"+parts[len(parts)-1], nil)
	if *gitlabUser != "" {
		req.SetBasicAuth(*gitlabUser, *gitlabToken)
	}
	...
	if address == (common.Address{}) {
		return "", "", common.Address{}, errors.New("No Usechain address found to fund")
	}
	...
	return gist.Owner.Login + "@gitlab", fmt.Sprintf("https://github.com/%s.png?size=64", gist.Owner.Login), address, nil
}
