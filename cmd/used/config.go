...
func loadConfig(file string, cfg *usedConfig) error {
	f, err := os.Open(file)
	if err != nil {
		return err
	}
...
func defaultNodeConfig() node.Config {
	...
	cfg.HTTPModules = append(cfg.HTTPModules, "use", "shh")
	cfg.WSModules = append(cfg.WSModules, "use", "shh")

	cfg.IPCPath = "used.ipc"
	return cfg
}

func makeConfigNode(ctx *cli.Context) (*node.Node, usedConfig) {
	// Load defaults.
	cfg := usedConfig{
