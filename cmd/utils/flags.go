...
import (
	...
	"github.com/usechain/go-usechain/p2p/netutil"
...
var (
	...
	NetworkIdFlag = cli.Uint64Flag{
		Name:  "networkid",
		Usage: "Network identifier (integer, 1=Frontier, 2=Moonet)",
...
func MakeDataDir(ctx *cli.Context) string {
	if path := ctx.GlobalString(DataDirFlag.Name); path != "" {
		...
		if ctx.GlobalBool(MoonetFlag.Name) {
			return filepath.Join(path, "moonet")
		}
		return path
	}
...
func setBootstrapNodes(ctx *cli.Context, cfg *p2p.Config) {
	...
	case ctx.GlobalBool(MoonetFlag.Name):
		urls = params.MoonetBootnodes
...
// setUsebase retrieves the usebase either from the directly specified
// command line flags or from the keystore if CLI indexed.
func setUsebase(ctx *cli.Context, ks *keystore.KeyStore, cfg *eth.Config) {
	if ctx.GlobalIsSet(UsebaseFlag.Name) {
		account, err := MakeAddress(ks, ctx.GlobalString(UsebaseFlag.Name))
		if err != nil {
			Fatalf("Option %q: %v", UsebaseFlag.Name, err)
		}
		cfg.Usebase = account.Address
	}
}
...
func SetP2PConfig(ctx *cli.Context, cfg *p2p.Config) {
	...
	log.Info("Maximum peer count", "USE", ethPeers, "LES", lightPeers, "total", cfg.MaxPeers)
...
func SetNodeConfig(ctx *cli.Context, cfg *node.Config) {
	...
	case ctx.GlobalBool(MoonetFlag.Name):
		cfg.DataDir = filepath.Join(node.DefaultDataDir(), "moonet")
	}
...
func SetEthConfig(ctx *cli.Context, stack *node.Node, cfg *eth.Config) {
	...
	checkExclusive(ctx, DeveloperFlag, TestnetFlag, MoonetFlag)
	checkExclusive(ctx, FastSyncFlag, LightModeFlag, SyncModeFlag)
	checkExclusive(ctx, LightServFlag, LightModeFlag)
	checkExclusive(ctx, LightServFlag, SyncModeFlag, "light")

	ks := stack.AccountManager().Backends(keystore.KeyStoreType)[0].(*keystore.KeyStore)
	setUsebase(ctx, ks, cfg)
	setGPO(ctx, &cfg.GPO)
	...
	case ctx.GlobalBool(MoonetFlag.Name):
		...
		cfg.Genesis = core.DefaultMoonetGenesisBlock()
	case ctx.GlobalBool(TestnetFlag.Name):
		if !ctx.GlobalIsSet(NetworkIdFlag.Name) {
			cfg.NetworkId = 3
		}
...
func RegisterEthService(stack *node.Node, cfg *eth.Config) {
	...
	if err != nil {
		Fatalf("Failed to register the Usechain service: %v", err)
	}
}
...
func RegisterEthStatsService(stack *node.Node, url string) {
	if err := stack.Register(func(ctx *node.ServiceContext) (node.Service, error) {
		...
		Fatalf("Failed to register the Usechain Stats service: %v", err)
	}
}
...
func MakeGenesis(ctx *cli.Context) *core.Genesis {
	...
	case ctx.GlobalBool(MoonetFlag.Name):
		genesis = core.DefaultMoonetGenesisBlock()
	case ctx.GlobalBool(DeveloperFlag.Name):
