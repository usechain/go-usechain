...
func setupTxPool() (*TxPool, *ecdsa.PrivateKey) {
	...
	if err != nil {
		log.Error("failed to generate one-shot node key: %v", err)
	}

	dir, err := ioutil.TempDir("", "node-test")
	if err != nil {
		log.Error("failed to create temporary data directory: %v", err)
	}
	defer os.RemoveAll(dir)

	keyfile := filepath.Join(dir, "unit-test", datadirPrivateKey)
	config := &node.Config{Name: "unit-test", DataDir: dir, P2P: p2p.Config{PrivateKey: key}}
	config.NodeKey()
	if _, err := os.Stat(filepath.Join(keyfile)); err == nil {
		log.Error("one-shot node key persisted to data directory")
	}

	newKey := keystore.NewPlaintextKeyStore(dir)
...
func (c *testChain) State() (*state.StateDB, error) {
	...
	if *c.trigger {
		...
		c.statedb.SetBalance(c.address, new(big.Int).SetUint64(params.Use))
		*c.trigger = false
	}
	return stdb, nil
}
...
func TestStateChangeDuringTransactionPoolReset(t *testing.T) {
	...
	statedb.SetBalance(address, new(big.Int).SetUint64(params.Use))
...
func testTransactionQueueGlobalLimiting(t *testing.T, nolocals bool) {
	...
	keyfile := filepath.Join(dir, "unit-test", datadirPrivateKey)
	conf := &node.Config{Name: "unit-test", DataDir: dir, P2P: p2p.Config{PrivateKey: key}}
	conf.NodeKey()
	if _, err := os.Stat(filepath.Join(keyfile)); err == nil {
		log.Error("one-shot node key persisted to data directory")
	}
