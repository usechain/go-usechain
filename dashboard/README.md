...
<<<<<<< HEAD
The dashboard is a data visualizer integrated into used, intended to collect and visualize useful information of an Ethereum node. It consists of two parts:
=======
...
>>>>>>> 132df76fbdb41db5196ac7edcd5b4988389b191e
...
<<<<<<< HEAD
Normally the dashboard assets are bundled into used via `go-bindata` to avoid external dependencies. Rebuilding used after each UI modification however is not feasible from a developer perspective. Instead, we can run `webpack` in watch mode to automatically rebundle the UI, and ask `used` to use external assets to not rely on compiled resources:

```
$ (cd dashboard/assets && ./node_modules/.bin/webpack --watch)
$ used --dashboard --dashboard.assets=dashboard/assets --vmodule=dashboard=5
```

To bundle up the final UI into used, run `go generate`:
=======
...
<<<<<<< HEAD
For more IDE support install the `linter-eslint` package too, which finds the `.eslintrc` file, and provides real-time linting. Atom warns, that these two packages are incompatible, but they seem to work well touseder. For third-party library errors and auto-completion [flow-typed][flow-typed] is used.
=======
