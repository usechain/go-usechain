...
#
# The cluster contains a bootnode, a used node and multiple swarm nodes, with
...
# static used configuration
used_IP="192.168.33.3"
used_RPC_PORT="8545"
used_RPC_URL="http://${used_IP}:${used_RPC_PORT}"
...
# start_bootnode starts a bootnode which is used to bootstrap the used and
# swarm nodes
start_bootnode() {
...
# start_used_node starts a used node with --datadir pointing at <base-dir>/used
# and a single, unlocked account with password "used"
start_used_node() {
  local dir="${base_dir}/used"
  mkdir -p "${dir}"
  ...
  if [[ -z "${address}" ]]; then
    fail "failed to get used account address"
  fi
  ...
  local args=(
    ...
    --rpc
    --rpcaddr   "${used_IP}"
    --rpcport   "${used_RPC_PORT}"
    --verbosity "6"
  )

  start_node "used" "${used_IP}" "$(which used)" ${args[@]}
}
...
create_account() {
  ...
  used --datadir "${dir}" --password /dev/stdin account new <<< "${password}"
}
